...
option_list = list(
  ...
  make_option("--ref_ld", action="store", default=NA, type='character',
              help="Prefix to reference LD files in binary PLINK format"),
  ...
  make_option("--TSS_file", action="store", default=NA, type='character',
              help="Path to Transcript Starting Site (TSS) file, (must have columns AptName(ID),Chr,TSS) [required]"),			  
...
# opt$sumstats <- '/mnt/lvm_vol_2/sliu/database/GWAS_summary/prostate_multiethnic_2023_NG_Anqi/prostate_East_Asian_2023_NG.txt'
# opt$out <- 'seq.4983.6_association.txt'
# opt$weights <- 'CHN_male/08_establish_prediction_models/pos/seq.4983.6.pos'
# opt$weights_dir <- 'CHN_male/08_establish_prediction_models/models/' 
# opt$ref_ld <- 'CHN_male/08_establish_prediction_models/ref_ld/seq.4983.6_1000G' 
# opt$TSS_file <- '01_preprocess_data/annotation/SomaScan_TSS.txt' 
...
sumstat = fread(opt$sumstats, data.table = F) #sumstat = read.table(opt$sumstats,head=T,as.is=T)
...
N = nrow(wgtlist)
#out.tbl = data.frame( "PANEL" = rep(NA,N) , "FILE" = character(N) , "ID" = character(N) ,"HSQ" = numeric(N) , "BEST.GWAS.ID" = character(N) , "BEST.GWAS.Z" = numeric(N) , "PQTL.ID" = character(N) , "PQTL.R2" = numeric(N) , "PQTL.Z" = numeric(N) , "PQTL.GWAS.Z" = numeric(N) , "NSNP" = numeric(N) , "NWGT" = numeric(N) , "MODEL" = character(N) , "MODELCV.R2" = character(N) , "MODELCV.PV" = character(N) , "PWAS.Z" = numeric(N) , "PWAS.P" = numeric(N) , stringsAsFactors=FALSE )
out.tbl = data.frame( "PANEL" = rep(NA,N) , "FILE" = character(N) , "ID" = character(N) ,"HSQ" = numeric(N),  "MODEL" = character(N) , "N_SNP" = numeric(N) , "N_cis" = numeric(N) , "N_trans" = numeric(N) , "MODELCV.R2" = character(N) , "MODELCV.PV" = character(N) , "PWAS.Z" = numeric(N) , "PWAS.P" = numeric(N) , stringsAsFactors=FALSE )
...
genos = read_plink(paste(opt$ref_ld,sep=''),impute="avg")
...
for ( w in 1:nrow(wgtlist) ) {
	...
	TSS <- read.table(opt$TSS, header = T)
    ...
    ID <- gsub('.wgt.RDat', '', wgtlist$WGT[w])
	...
	if ( !cur.FAIL ) {
		...
		out.tbl$N_SNP[w] = N_SNP
		out.tbl$N_cis[w] = N_cis
		out.tbl$N_trans[w] = N_trans
	...
	# perform COLOC test
	if ( !is.na(opt$coloc_P) && !is.na(out.tbl$PWAS.Z[w]) && out.tbl$PWAS.P[w] < opt$coloc_P && !is.na(wgtlist$N[w]) ) {
