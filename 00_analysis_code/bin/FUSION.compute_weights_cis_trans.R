...
weights.bslmm = function( input , bv_type , snp , out=NA ) {
	...
	eff = read.table( paste("output/",out,".param.txt",sep=''),head=T,as.is=T) # eff = read.table( paste(out,".param.txt",sep=''),head=T,as.is=T)
...
if ( hsq.pv > opt$hsq_p ) {  #if ( hsq[1] < 0 || hsq.pv > opt$hsq_p ) {  
